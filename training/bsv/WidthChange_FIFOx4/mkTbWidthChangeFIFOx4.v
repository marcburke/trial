...
//
// On Tue Nov  3 13:13:24 EST 2015
//
//
// Ports:
// Name                         I/O  size props
...
module mkTbWidthChangeFIFOx4(CLK,
			     RST_N);
  input  CLK;
  input  RST_N;
  ...
  begin
	...
	$display("BRAM01 Read = %x %x",
		 $unsigned(wc_fifo$pio_0_response_get),
		 $unsigned(wc_fifo$pio_1_response_get));
    if (RST_N != `BSV_RESET_VALUE)
	...
	$display("BRAM23 Read = %x %x",
		 $unsigned(wc_fifo$pio_2_response_get),
		 $unsigned(wc_fifo$pio_3_response_get));
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_done) $display("TESTS FINISHED");
    if (RST_N != `BSV_RESET_VALUE) if (WILL_FIRE_RL_done) $finish(32'd0);
  end
  // synopsys translate_on
endmodule  // mkTbWidthChangeFIFOx4
